...
const Booking = () => {
    ...
    const { data: availableSlots } = useCheckAvailabilityQuery({ date: selectedDate, facility: id }, { skip: !id && !selectedDate })
            ...
            <div className="max-w-4xl mx-auto p-5 bg-white shadow-2xl rounded-3xl">
                <div className='flex flex-col md:flex-row lg:flex-row gap-y-3 gap-x-3 justify-between my-3'>
                    <div className=' w-full md:w-1/2 lg:w-1/2'>
                        <img className='w-full h-[250px] rounded-lg' src={facility.image} alt="" />
                    </div>
                    <div className='w-full md:w-1/2 lg:w-1/2'>
                        <h1 className="text-2xl font-bold mb-3">{facility.name} Booking</h1>
                        <p className='font-medium text-slate-500 mb-4'>{facility.description}</p>
                        ...
                        <div className='flex gap-x-2 items-center my-1'>
                            ...
                            <p className='font-semibold text-blue-500 text-2xl flex items-center gap-x-1'>${facility.pricePerHour} <span className='text-sm text-black'>/ per hour</span></p>
                        </div>
                    </div>
                </div>
                            ...
                            {availableSlots?.data?.map((slot: TAvailableSlots) => (
                                ...
                                >
                                    <div>
                                        ...
                                        <p className='text-lg font-semibold text-blue-500'>{slot.startTime}</p>
                                    </div>
                                    <div>
                                        <p className='font-medium'>End Time</p>
                                        <p className='text-lg font-semibold text-red-500'>{slot.endTime}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ) : (
                        <p className="text-center text-gray-500">Please select a date to see available slots.</p>
                    )}
                </div>

                {selectedDate && (
                    <div className="mt-8 text-center">
                        <button onClick={() => (document.getElementById('bookingModal') as HTMLDialogElement)!.showModal()} className="px-8 py-3 bg-blue-500 text-white rounded-full shadow-lg hover:bg-blue-600 transition-transform transform hover:scale-105 text-lg font-bold">
                            Confirm Booking
                        </button>
                    </div>
                )}
